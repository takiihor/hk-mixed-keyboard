...
class CandidateDisplayPolicy {
    ...
    fun order(
        ...
        val mixed = rest.filter { it.type == CandidateType.MIXED_PHRASE }
        val nonMixed = rest.filterNot { it.type == CandidateType.MIXED_PHRASE }
        ...
        val learnedEnglish = learned
            ...
            .map { it.candidate.asLearnedCandidate() }
        val decodedChinese = nonMixed.filter(::isChinese)
        val decodedEnglish = nonMixed.filterNot(::isChinese)
        ...
        val ordered = if (chineseFirst || buffer.length <= 2) {
            ...
            custom + exactEnglishAssist + mixed + learnedEnglish + english + literal +
    ...
    fun orderPredictions(
        ...
        val learnedChinese = learned
            .filter { isChinese(it.candidate) }
            .map { it.candidate.asLearnedCandidate() }
        val translations = decoded.filter {
            it.sourceSchema == SourceSchema.CHINESE_ASSIST
        }
        val predictions = decoded.filter(::isChinese)

        return (learnedChinese + predictions + translations)
            .distinctBy { it.text }
            .take(limit)
    }
    ...
    private fun isChinese(candidate: DecodeCandidate): Boolean =
        candidate.type != CandidateType.EN_LITERAL &&
            candidate.type != CandidateType.EN_AUTOCOMPLETE &&
            candidate.type != CandidateType.CHINESE_ASSIST
